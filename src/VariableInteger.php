        ...
        switch ($encodingType) {
            ...
            case 1: // 2字节编码
                ...
                if (false === $unpacked || !isset($unpacked[1]) || !is_int($unpacked[1])) {
                    throw VariableIntegerException::insufficientData('数据格式错误，无法解码2字节变长整数');
                }
                $value = $unpacked[1];
            ...
            case 2: // 4字节编码
                ...
                if (false === $unpacked || !isset($unpacked[1]) || !is_int($unpacked[1])) {
                    throw VariableIntegerException::insufficientData('数据格式错误，无法解码4字节变长整数');
                }
                $value = $unpacked[1];
            ...
            case 3: // 8字节编码
                ...
                $unpacked = unpack('N2', substr($data, $offset, 8));
                if (false === $unpacked || !isset($unpacked[1], $unpacked[2])
                    || !is_int($unpacked[1]) || !is_int($unpacked[2])) {
                    throw VariableIntegerException::insufficientData('数据格式错误，无法解码8字节变长整数');
                }
                $high = $unpacked[1] & 0x3FFFFFFF;
                $low = $unpacked[2];
